...
```powershell
./init-databases.ps1   # 一次性初始化文章库、用户库，并清空 MongoDB 内容
```

脚本会自动查找运行中的 PostgreSQL 与 MongoDB 容器，复制 `database/sql` 目录下的 SQL 文件到容器内执行，最后重建示例数据。各服务目录下遗留的初始化脚本已改为兼容入口，内部会调用本脚本。
