# 博客平台 - 微服务集成指南

## 📋 概述

本文档描述了 blog-user-service 和 blog-auth-service 的完整集成方案，实现了：

- 用户注册与认证分离
- OpenFeign 跨服务调用
- JWT Token 生成与验证
- Redis 黑名单机制

## 🏗️ 架构设计

### 服务职责划分

```
┌─────────────────────────────────────────────────────────────┐
│                        客户端/前端                            │
└─────────────────────────────────────────────────────────────┘
                              │
                 ┌────────────┴────────────┐
                 │                         │
                 ▼                         ▼
    ┌────────────────────┐    ┌────────────────────┐
    │ blog-user-service  │    │ blog-auth-service  │
    │   (端口 8083)      │◄───│   (端口 8081)      │
    │                    │    │                    │
    │ 职责:              │    │ 职责:              │
    │ • 用户注册         │    │ • 用户登录         │
    │ • 用户信息管理     │    │ • JWT 生成         │
    │ • 认证凭证存储     │    │ • Token 验证       │
    │                    │    │ • Token 黑名单     │
    └────────┬───────────┘    └────────┬───────────┘
             │                         │
             ▼                         ▼
    ┌─────────────────┐      ┌─────────────────┐
    │   PostgreSQL    │      │      Redis      │
    │  (端口 15432)   │      │   (端口 6379)   │
    │                 │      │                 │
    │ • t_user        │      │ • JWT 黑名单    │
    │ • t_user_auth   │      │                 │
    └─────────────────┘      └─────────────────┘
```

### 调用流程

#### 1️⃣ 用户注册流程

```
客户端 → blog-user-service
         └─> 验证数据
         └─> BCrypt 加密密码
         └─> 存储到 PostgreSQL
         └─> 返回用户信息
```

#### 2️⃣ 用户登录流程

```
客户端 → blog-auth-service
         └─> [Feign Client] 调用 blog-user-service
                           └─> 查询用户认证信息
                           └─> 返回加密密码
         └─> BCrypt 验证密码
         └─> 生成 JWT (Access + Refresh)
         └─> 返回 Tokens
```

#### 3️⃣ Token 验证流程

```
客户端 → blog-auth-service /validate
         └─> 验证 JWT 签名
         └─> 检查 Redis 黑名单
         └─> 返回用户名
```

#### 4️⃣ 用户登出流程

```
客户端 → blog-auth-service /logout
         └─> 提取 Token JTI
         └─> 加入 Redis 黑名单 (TTL = Token 剩余时间)
         └─> 返回成功
```

## 🔧 技术实现

### Feign Client 集成

**UserServiceClient.java**

```java
@FeignClient(
    name = "blog-user-service",
    url = "${user-service.url:http://localhost:8083}",
    fallback = UserServiceClientFallback.class
)
public interface UserServiceClient {
    @GetMapping("/api/users/internal/auth-details")
    UserAuthDetailsDTO getUserAuthDetails(@RequestParam("identifier") String identifier);
}
```

**配置文件 (application.yml)**

```yaml
user-service:
  url: ${USER_SERVICE_URL:http://localhost:8083}

feign:
  client:
    config:
      default:
        connectTimeout: 5000
        readTimeout: 5000
        loggerLevel: basic
```

### UserDetailsService 集成

**修改前 (Mock 实现)**

```java
if ("user".equals(username)) {
    return new User("user", passwordEncoder.encode("password"), new ArrayList<>());
}
```

**修改后 (Feign 调用)**

```java
UserAuthDetailsDTO authDetails = userServiceClient.getUserAuthDetails(username);
return User.builder()
    .username(authDetails.getIdentifier())
    .password(authDetails.getCredential())  // 已加密的密码
    .authorities(new ArrayList<>())
    .disabled(authDetails.getStatus() != 1)
    .build();
```

## 🚀 快速开始

### 1. 启动基础设施

```powershell
# Redis
docker run -d --name redis -p 6379:6379 redis:7-alpine

# PostgreSQL (如果还未初始化)
cd blog-user-service
.\init-db.ps1
```

### 2. 启动服务

**方式 A: 自动启动所有服务**

```powershell
.\start-all-services.ps1
```

**方式 B: 手动逐个启动**

```powershell
# 终端 1: 启动用户服务
cd blog-user-service
mvn spring-boot:run

# 终端 2: 启动认证服务
cd blog-auth-service
mvn spring-boot:run
```

### 3. 运行集成测试

```powershell
.\test-integration.ps1
```

## 🧪 测试场景

集成测试脚本验证以下流程：

| 步骤 | 操作       | 验证点               |
| ---- | ---------- | -------------------- |
| 1    | 用户注册   | PostgreSQL 存储成功  |
| 2    | 查询用户   | 数据一致性           |
| 3    | 用户登录   | Feign 调用、JWT 生成 |
| 4    | Token 验证 | 签名验证通过         |
| 5    | 用户登出   | Redis 黑名单添加     |
| 6    | 黑名单验证 | Token 被拒绝 (401)   |

### 预期输出

```
✓ 用户注册 (PostgreSQL 存储)
✓ 用户查询
✓ auth-service 调用 user-service (Feign Client)
✓ BCrypt 密码验证
✓ JWT Token 生成
✓ JWT Token 验证
✓ Redis 黑名单机制
✓ 黑名单 Token 拒绝
```

## 📊 数据流

### 注册时的密码加密

```
客户端密码 (明文)
    ↓
blog-user-service (BCrypt 加密)
    ↓
PostgreSQL t_user_auth (存储哈希)
```

### 登录时的密码验证

```
客户端密码 (明文)
    ↓
blog-auth-service
    ↓
Feign → blog-user-service (获取哈希密码)
    ↓
Spring Security (BCrypt.matches)
    ↓
JWT 生成
```

## 🔐 安全特性

### 1. 密码安全

- ✅ BCrypt 加密 (默认 10 轮)
- ✅ 密码明文永不存储
- ✅ 密码永不通过网络传输（除初始注册/登录）

### 2. Token 安全

- ✅ JWT 使用 HMAC-SHA256 签名
- ✅ Access Token: 1 小时过期
- ✅ Refresh Token: 7 天过期
- ✅ 登出后立即失效 (Redis 黑名单)

### 3. 服务间通信

- ✅ 内部接口 `/internal/*` 应配置网关拦截
- ✅ Feign 超时配置 (5 秒)
- ✅ 降级处理 (UserServiceClientFallback)

## 📝 API 端点

### blog-user-service (8083)

| 端点                               | 方法 | 说明                |
| ---------------------------------- | ---- | ------------------- |
| `/api/users/register`              | POST | 用户注册            |
| `/api/users/{id}`                  | GET  | 获取用户信息        |
| `/api/users/internal/auth-details` | GET  | 获取认证详情 (内部) |

### blog-auth-service (8081)

| 端点                 | 方法 | 说明       |
| -------------------- | ---- | ---------- |
| `/api/auth/login`    | POST | 用户登录   |
| `/api/auth/logout`   | POST | 用户登出   |
| `/api/auth/refresh`  | POST | 刷新 Token |
| `/api/auth/validate` | GET  | 验证 Token |

## 🐛 故障排查

### 问题 1: Feign 调用失败

**症状**: `Connection refused` 或超时

**解决方案**:

1. 检查 user-service 是否启动: `curl http://localhost:8083/actuator/health`
2. 检查网络连接
3. 查看 auth-service 日志中的 Feign 错误

### 问题 2: 登录返回 401

**症状**: 认证失败

**可能原因**:

- 用户未注册
- 密码错误
- 用户状态为 0 (inactive)

**调试步骤**:

```powershell
# 检查用户是否存在
curl http://localhost:8083/api/users/internal/auth-details?identifier=test@example.com

# 查看认证服务日志
# 应该看到 "Loading user details for username: xxx"
```

### 问题 3: Token 验证失败

**症状**: `/validate` 返回 401

**可能原因**:

- Token 已过期
- Token 在黑名单中
- JWT 签名不匹配

**调试**:

```powershell
# 检查 Redis 黑名单
docker exec -it redis redis-cli
> KEYS jwt:blacklist:*
> GET jwt:blacklist:<jti>
```

## 📈 性能优化建议

### 1. Redis 连接池

```yaml
spring:
  data:
    redis:
      lettuce:
        pool:
          max-active: 8
          max-idle: 8
          min-idle: 2
```

### 2. Feign 连接池

```xml
<dependency>
    <groupId>io.github.openfeign</groupId>
    <artifactId>feign-httpclient</artifactId>
</dependency>
```

### 3. 数据库连接池

```yaml
spring:
  datasource:
    hikari:
      maximum-pool-size: 10
      minimum-idle: 5
```

## 🔄 未来扩展

- [ ] 添加服务发现 (Eureka)
- [ ] 集成 API Gateway
- [ ] 添加分布式追踪 (Zipkin)
- [ ] 实现 OAuth2 第三方登录
- [ ] 添加用户权限管理 (RBAC)

## 📚 相关文档

- [blog-user-service README](blog-user-service/README.md)
- [blog-auth-service README](blog-auth-service/README.md)
- [项目分析报告](project-analysis-report.md)

## ✅ 总结

通过 OpenFeign 集成，实现了：

1. ✅ 服务职责分离（用户管理 vs 认证授权）
2. ✅ 跨服务通信（声明式 REST 客户端）
3. ✅ 统一密码加密（BCrypt）
4. ✅ JWT 生命周期管理（生成、验证、撤销）
5. ✅ Redis 黑名单机制
6. ✅ 完整的端到端测试

现在两个服务已完美集成，可以支撑完整的用户注册、登录、认证流程！
