...
- **技术栈**:
  ...
  - **消息队列**: Spring AMQP (RabbitMQ) 用于点赞/评论通知异步化。
...
- **技术栈**:
  ...
  - **数据访问**: MyBatis-Plus。
  - **消息队列**: Spring AMQP 消费 RabbitMQ 消息生成互动通知。
...
3. 服务会通过内置的 RabbitMQ Publisher 将 `ArticleInteractionMessage` 投递到 `blog.notification.events` 队列，避免跨服务同步调用带来的延迟与耦合。
4. 用户服务订阅同一队列，消费文章互动消息并落库到 `t_user_notification`；随后通过 `/api/notifications/me` 等端点向前端暴露通知数据，`notificationsStore` 负责管理未读状态和批量已读，保证互动体验闭环。
...
- **`init-databases.ps1`**：根目录提供的一键初始化脚本，会定位正在运行的 PostgreSQL、MongoDB 容器，将 `database/sql` 目录下的 `article-db-init.sql`、`user-db-init.sql` 复制到容器内执行，并在最后清空 MongoDB、校验表结构。旧的服务级脚本（如 `blog-article-service\init-db-docker.ps1`）现在只是兼容入口，内部会转调该脚本。
- **SQL 与脚本分离**：所有结构化数据的重建逻辑均移动到 `database/sql`，PowerShell 仅保留控制流程，避免再次出现 psql 将脚本提示识别为 SQL 的问题。
- **`start-all-services.ps1`**：结合端口检测、Eureka 启动与服务编译的自动化脚本，可在数据库初始化后直接拉起三个后端 + Vue 前端，支持本地与 Docker 混合运行场景。
