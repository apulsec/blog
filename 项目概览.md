...
本文档旨在全面分析 `blog` 项目的架构、模块功能和技术栈。该项目是一个基于微服务架构的博客平台，由一个前端应用程序和三个后端服务组成。

## 1. 项目整体架构

本项目采用前后端分离的微服务架构，具体包含以下四个核心模块：
...
- **`blog-auth-service`** 在验证用户登录时，会调用 **`blog-user-service`** 的内部接口来获取用户的认证凭证（加密后的密码）。
- **`blog-article-service`** 在展示文章时，会调用 **`blog-user-service`** 来获取作者的详细信息。
...
- **文件结构与功能**:
  ...
  - `src/main/resources/application.yml`: 核心配置文件。
    ...
    - 配置了 Eureka 客户端，但默认禁用 (`EUREKA_ENABLED: false`)，方便独立测试。
